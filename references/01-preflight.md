...
- merge-base 是判断"分支侧改了什么 / 主干侧改了什么"的分界点，Phase 2 强依赖它。

### 0.5 建立产物目录与回退锚点

- `<分支名>` 中的 `/`（如 `feature/x`）替换为 `-`（→ `feature-x`），避免建出多级目录。
- 建目录并初始化进度面板（基于 `templates/ALIGN_PROGRESS.md`）：
  ```bash
  mkdir -p .qiqskills/<分支名>
...
```bash
git stash push -u -m "qiq-alignmain:<分支名>:<YYYYMMDD-HHMMSS>"
